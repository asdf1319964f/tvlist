...
def get_classes(rec):
    ...
    if rec.get('url') and str(rec['url']).startswith('http'):
        ...
        try:
            ...
            print(f'获取资源【{rec.get("name") or rec.get("type_name")}】({_api})分类发生错误:{e}')
...
def get_convert_classes(rec):
    ...
    name = rec.get("type_name") or rec.get("name")
    classes = convert_class(classes, name)
    return classes
...
def main(fname='采集'):
    ...
    out_file_path = file_path.replace('.json', '.js')
    ...
    for i, record in enumerate(records):
        rec_name = record.get("type_name") or record.get("name")
        if rec_name:
             has_name = [ret for ret in results if ret.get("name") == rec_name]
             if has_name:
                 record.update(has_name[-1])
                 js_output += "    {\n"
                 js_output += f"      title: '{rec_name}',\n"
                 js_output += f"      模板: '采集1',\n"
                 js_output += f"      host: '{record['url']}',\n"
                 if record.get('api'):
                     js_output += f"       api: '{record['api']}',\n"
                 else:
                     js_output += f"       api: '/api.php/provide/vod/',\n"
                 js_output += f"      parse_url: '',\n"
                 js_output += f"      header: '',\n"
                 if record.get('class_name') :
                    js_output += f"      class_name: '{record['class_name']}',\n"
                 if record.get('class_url') :
                      js_output += f"      class_url: '{record['class_url']}',\n"
                 js_output += f"      cate_exclude: '',\n"
                 js_output += f"      play_parse: false,\n"
                 js_output += f"      search: false,\n"
                 js_output += f"      force_http: false,\n"
                 js_output += "    },\n"
    js_output += "  ]\n};"

    with open(out_file_path, mode='w+', encoding='utf-8') as f:
        f.write(js_output)
    print(f'转换静态数据成功,文件路径:{out_file_path}')
...
def main_exclude(fname='采集静态', max_workers=0):
    ...
    for rec in records:
        ...
        if not check_active(api_url):
            print(f'{rec.get("type_name") or rec.get("name")} ({rec["url"]})视为不存活,跳过分类检测')
        else:
        ...
        js_output += "    {\n"
        js_output += f"      title: '{rec.get("type_name") or rec.get("name")}',\n"
        js_output += f"      模板: '采集1',\n"
        js_output += f"      host: '{rec['url']}',\n"
        if rec.get('api'):
            js_output += f"       api: '{rec['api']}',\n"
        else:
            js_output += f"       api: '/api.php/provide/vod/',\n"
        js_output += f"      parse_url: '',\n"
        js_output += f"      header: '',\n"
        if rec.get('class_name') :
            js_output += f"      class_name: '{rec['class_name']}',\n"
        if rec.get('class_url') :
            js_output += f"      class_url: '{rec['class_url']}',\n"
        if len(cate_excludes) > 0 :
             js_output += f"      cate_exclude: '{'&'.join(cate_excludes)}',\n"
        else:
            js_output += f"      cate_exclude: '',\n"
        js_output += f"      play_parse: false,\n"
        js_output += f"      search: false,\n"
