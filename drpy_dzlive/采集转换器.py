...
def main(zy_url="https://cdn.jsdelivr.net/gh/waifu-project/v1@latest/yoyo.json"): #修改这里为本地文件
    try:
        # 使用本地文件
        with open(zy_url, 'r', encoding='utf-8') as f:
            ret = json.load(f)
        ...
        covert_sites = []
        for site in sites:
            site_name = site.get("name") or site.get("type_name")
            if site_name and site.get("api"):
                surl = site['api']
                host = get_host(surl)
                api = get_api(surl)
                sid = get_sid(surl)
